...
contract("CaptureTheFlag", async (accounts) => {
  ...
  it("Runs with GSN", async () => {
    ...
    assert.equal(
      res.logs[0].args.previousHolder,
      ZERO_ADDRESS,
      "Wrong previous flag holder"
    );
    assert.equal(
      res.logs[0].args.currentHolder,
      account,
      "Wrong current flag holder"
    );
  });

  it("Paymaster should reject different account", async () => {
    await captureFlagContract
      .captureTheFlag({ from: accounts[2] })
      .then(() => {
